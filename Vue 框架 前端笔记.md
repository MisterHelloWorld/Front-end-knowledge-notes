...
**补充：可以单独将组件引入到某个文件夹下的 index.js 文件中，并按需导出**

```javascript
// 单独引入
export { default as 组件名重命名 } from '组件路径'
// 按需导入组件
import { 重命名的组件 } from 'index.js文件所在的文件夹'
```
...
**当不可避免操作 DOM 的时候（如拖拽、监听 img 标签图片加载失败并做相关处理），可以考虑使用自定义指令**
...
2. **在 index.js 文件中，必须先提前引入 vue 包（ import Vue from ' vue ' ），然后注册全局的自定义指令**
   ...
   Vue.directive('指令名称', {
     ...
     inserted(el, options) {
       ...
       处理程序（如监听图片加载错误的onerror事件，该事件会在图片加载错误时自动触发）
     }
   })
   ```
